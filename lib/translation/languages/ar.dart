Map<String, String> arabic = {
  //1.replace : "" => :"
  //2. (?<=:").([^"]*) => nothing

  //pages
  "home": "الرئيسية",
  "transactions": "تحويلاتي",
  "aboutUs": "معلومات عنا",
  "reviews": "التقييمات",
  //App Values
  "youCanNowTransferMoneyEasly":
      "تم إنشاء خدمة تحويل العملات المشفرة لتسهيل تحويل وتبادل الأموال بين محافظ التشفير والسحب في سوريا",
  "transferingMoneyMadeSimple": "يمكنك الآن تحويل الأموال بسهولة",
  "sendMoneyToAnyOneOrTransferYourBalanceToMoneyWithUs":
      "الآن يمكنك تحويل أموالك إلى محافظ مختلفة بسهولة وسحبها بشكل آمن",

  //Common
  "transferNow": "تحويل الآن",
  "email": "البريد الالكتروني",
  "typeInYourEmail": "أدخل البريد الالكتروني",
  "password": "كلمة المرور",
  "typeInYourPassword": "أدخل كلمة المرور",
  "thisFieldIsRequired": "هذا الحقل مطلوب",
  "passwordCanNotBeLessThanSixCharacters":
      "كلمة المرور يجب أن تتألف على الأقل من 6 ",
  "firstName": "الاسم الأول",
  "lastName": "الاسم الأخير",
  "emailMustBeLike": "البريد الالكتروني يجب أن يكون من الشكل",

  //Login
  "login": "تسجيل الدخول",
  "loginToYourAccount": "تسجيل الخول إلى حسابك",
  "forgotYourPassword?": "نسيت كلمة المرور؟",
  "reset": "استعادة",
  "dontHaveAnAccount?": "ليس لديك حساب بعد",

  //Register
  "registerNewAccount": "إنشاء حساب جديد",
  "register": "التسجيل",
  "typeInYourFirstName": "أدخل الاسم الأول",
  "firstNameMustBeThree": "الاسم الأول يجب أن يتألف على الأقل من ثلاث حروف",
  "typeInYourLastName": "أدخل الاسم الأخير",
  "lastNameMustBeThree": "الاسم الأخير يجب أن يتألف على الأقل من ثلاث حروف",
  "invalidPassword": "كلمة المررو غير صالحة",
  "typeInPasswordConfirmation": "أدخل تأكيد كلمة المرور",
  "passwordAndPasswordConfirmationAreNotTheSame":
      "كلمة المرور وتأكيد كلمة المرور غير متطابقين",
  "registerSuccess": " تم ارسال الرمز الى البريد الكتروني  ",
  "confirmPassword": "تأكيد كلمة المرور",
  "alreadyHaveAnAccount?": "لديك حساب بالفعل ؟",

  //Enter Email / reset pass
  "invalidEmail": "بريد الكتروني غير صالح",
  "enterEmailToSendTheCode": "أدخل البريد الالكتروني لإرسال رمز التأكيد",
  "resetPassword": "استعادة كلمة المرور",
  "submit": "إرسال",

  //OTP
  "codeMustBeSixNumbers": "الرمز يجب أن يكون مؤلف من ست خانات",
  "verificationCode": "رمز التأكيد",
  "otpWrong": "رمز التأكيد خطأ",
  "pleaseTypeInTheCodeSentToYourEmail":
      " من فضلك أدخل الرمز الذي تم إرساله إلى بريدك الاكتروني يمكن التحقق من قسم الرسائل غير المرغوب بها",
  "verify": "تحقق",
  "haventRecievedACode?": "لم تتلق رمز التأكيد",
  "resend": "إعادة الإرسال",

  //Update Password
  "passwordUpdatedSuccessfully": "تم تحديث كلمة المرور بنجاح",
  "enterNewePassword": "أدخل كلمة المرور الجديدة",
  "updatePassword": "تحديث كلمة المرور",

  //Profile
  "name": "اسم الستخدم",
  "logout": "تسجيل الخروج",
  "changeLangauge": "تغيير اللغة",
  "profile": "الملف الشخصي",

  //Footer
  "downloadTheAppByClickingTheLinkBelow":
      "حمل التطبيق من خلال الرابط في الأسفل",
  "contactUs": "تواصل معنا",
  "ourPages": "صفحاتنا الرسمية",

  //Transfer
  "waitInfoLoading": "انتظر تحميل المعلومات",
  "back": "عودة",
  "nextStep": "التالي",

  "submitApplication": "إرسال الطلب",
  "areYouSureAboutInfoYouEntered":
      "هل أنت متأكد من المعلومات التي قمت بإدخالها",
  "yesIamSure": "نعم، أنا متأكد",
  "noEdit": "لا ، تعديل المعلومات",
  "successOperation": "تم إرسال الطلب للمعالجة",
  "error": "حدث خطأ",
  "pleaseFillAllFields": "من فضلك املأ جميع الحقول",

  //Transfer first step
  "chooseAmountAndWallet": "اختر المحفظة والكمية",
  "chooseWalletWhichYouWillTransferFrom": "اختر المحفظة التي ستحول منها ",
  "chooseWalletWhichYouWillTransferTo": "اختر المحفظة التي ستحول إليها",
  "minAmountCouldBeTransfered": "أقل كمية ممكنة للتحويل",
  "maxAmountCouldBeTransfered": "أكبر كمية ممكنة للتحويل",
  "enterAmount": "أدخل المبلغ",
  "amountWillBeAfterCutFee": "استلام المبلغ بعد خصم العمولة منه بنسبة",
  "amountMustBeAtLeast": "المبلغ يجب أن يكون على الأقل",
  "amountMustBeAtMost": "المبلغ يجب أن يكون على الأكثر",
  "pleaseEnterOnlyNumbers": "من فضلك أدخل أرقام صحيحة فقط",
  "selectWallet": "اختر محفظة",
  "cash": "عملة البلد أو الدولار",
  "exchangeValue": "القيمة حسب الصرف بالليرة السورية",
  "syrianBound": "ليرة سورية",
  "syriatel-money": "سيرياتل كاش",
  //Transfer second step
  "yourDetails": "تفاصيلك",
  "yourEmail": "بريدك الالكتروني",
  "enterYourPhoneConsistOf10DigitsAndBeginWith09": "أدخل رقم هاتفك ",
  "enterYourFullNameAsItOnId": "أدخل اسمك الثلاثي كما هو موجود على الهوية",
  "enterYourCityName": "أدخل اسم بلدك ومدينتك",
  "enterYourAccountId": "أدخل عنوان محفظتك",
  "exchangeConfirmation": "تأكيد التحويل",

  //Transfer third step
  "our": "الخاص بنا",
  "accountDetails": "تفاصيل حساب",
  "pleaseMakePaymentToOurAccountAndEnterYourPaymentDetailsInTheRelatedField":
      "يرجى إجراء الدفع لحسابنا وإدراج تفاصيل الدفع الخاصة بك في الحقل أدناه",
  "ourAccountId": "عنوان محفظتنا",
  "codeCoppied": "تم نسخ رمز المحفظة",
  "infoCopied": "تم نسخ المعلومات إلى الحافظة ",
  "enterPaymentCodeThatYouDid": "أدخل تفاصيل الدفع الخاصة بك",
  "transferMoneyAsCashByHaramAndAddTranferReceiptInTheFieldBelow":
      "أرسل المبلغ عن طريق شركة الهرم أو مكاتب التحويل باستخدام المعلومات التالية مع إرفاق نسخة من إيصال التحويل في الحقل المخصص أو تواصل معنا عبر الواتساب",
  "ourTransferInfo": "معلومات التحويل الخاصة بنا",
  "recieverName": "اسم المتلقي",
  "receiverPhone": "رقم الهاتف",
  "receiverCity": "المدينة",
  "chooseReceiptImage": "اختر صورة وصل التحويل",
  "pleaseChooseReceiptImage": "يرجى اختيار صورة وصل التحويل",
  "pickReceiptImage": "اختر صورة الوصل",
  "confirm": "تأكيد",

  //Transactions Table :
  "id": "معرف المرجع",
  "date": "التاريخ",
  "send": "المرسل",
  "receive": "الاستلام",
  "status": "الحالة",
  "pending": "قيد التنفيذ",
  "done": "مؤكد",
  "notDone": "ملغاة",
  "details": "تفاصيل",
  //Transaction details
  "from": "من",
  "to": "إلى",
  "payPrice": "سعر الصرف",
  "account": "الحساب",
  "paymentDetails": "بيانات الدفع",
  "operationDate": "تاريخ العملية",
  "operationId": "معرف العملية",
  "payDate": "تاريخ الصرف",
  "statusMessage": "رسالة الحالة",
  "transferOPerationToYourAccountId": "معرف عملية التحويل إلى حسابك",
  "transferOPerationToYourAccountImage": "صورة وصل التحويل إلى حسابك",
  //reviews
  "ratingAndReviewsPageAreVerifiedAndFromPeopleWhoUseOurServices":
      "هذه التقييمات تم التحقق منها وهي صادرة عن أشخاص قاموا بطلب خدماتنا.",
  "addReview": "أضف تقييماً",
  //review-card
  "showLess": "أقل",
  "showMore": "المزيد",
  //add-review
  "ok": "موافق",
  "cancel": "إلغاء",
  "reviewAdded": "تم إضافة تقييمك",
};
